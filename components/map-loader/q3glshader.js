import basic from './basis/basis-basics.js';
...
q3glshader.init = function (gl, lightmap)
{
	...
	image.src = '/demo_baseq3/webgl/no-shader.png';
...
{
	// 1. Strip the extension out of the initial URL path to isolate the base name
	const baseName = url.replace(/\.[^/.]+$/, "");

	// 2. Define your target formats in order of preference
	...
	image.onerror = function ()
	{
		...
		{
			const nextExt = extensions[attemptIndex];

			if(nextExt === '.tga')
			{
				// Hook your binary TGA parser directly into the failure chain
				console.log(`Image failed. Dropping back to custom binary TGA parser for: ${baseName}.tga`);
				// executeTgaFallback(gl, stage, baseName + '.tga', onload);
				return;
			}
		...
		{
			console.error(`WebGL Texture Pipeline: All fallback paths exhausted for asset: ${url}`);
			// Optional: load a default magenta missing-texture fallback block here so the map doesn't crash
		}
	};

	// Determine starting index based on what came in, or default to 0 (.png)
	const currentExt = url.substring(url.lastIndexOf('.')).toLowerCase();
