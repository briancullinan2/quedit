...
q3bsp.prototype.buildBuffers = function (vertices, indices)
{
	...
	// FIX: Change Uint16Array to Uint32Array to support > 65k vertices
	...
	gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint32Array(skyIndices), gl.STATIC_DRAW);
...
q3bsp.prototype.bindShaders = function ()
{
	...
	{
		if(map.unshadedSurfaces.length === 0)
			...
			if(q3glshader.setShader(gl, this.skyShader))
			{
				for(var j = 0; j < this.skyShader.stages.length; ++j)
				{
					...
					for(var v = 0; v < viewCount; ++v)
					{
						...
						gl.drawElements(gl.TRIANGLES, this.skyboxIndexCount, gl.UNSIGNED_INT, 0);
					}
				}
			}
		}
		...
		if(this.defaultSurfaces.length > 0 || this.unshadedSurfaces.length > 0)
		{
			...
			for(var v = 0; v < viewCount; ++v)
			{
				...
				for(i = 0; i < this.unshadedSurfaces.length; ++i)
				{
					...
					gl.drawElements(gl.TRIANGLES, surface.elementCount, gl.UNSIGNED_INT, surface.indexOffset);
				}
