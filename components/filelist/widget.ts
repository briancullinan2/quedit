...
export class FileListWidget extends Widget {
    ...
    private async onRepositoryChanged(): Promise<void> {
        ...
        this.treeInstance = new Tree(targetTreeElement, {
            data: (window as any).convertFlatToNested(Object.values(treeData)),
            autoOpen: false,
            closeDepth: 2,
        });
    }
    ...
    private async loadGitHubTree(owner: string, repo: string, branch: string): Promise<any> {
        ...
        try {
            ...
            const treeData = await (window as any).githubRequest(owner, repo, `git/trees/${branch}?recursive=1`);
            const commitData = await (window as any).githubRequest(owner, repo, `commits/${branch}`);
    ...
    private dispatchFileActivation(filePath: string): void {
        ...
        // Invoke the global menu controller directly to safely build the new canvas context tab
        if ((window as any).triggerPanelRoute) {
            (window as any).triggerPanelRoute(targetRoute, (window as any).appMainDockPanel);
        }
    }
